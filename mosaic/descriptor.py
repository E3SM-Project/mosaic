...
def _compute_vertex_patches(ds):
    ...
    # if cell and edge missing collapse cell node to vertex, else leave as is
    nodes[:, 1::2, 0] = np.where(unionMask, xVertex, nodes[:, 1::2, 0])
    nodes[:, 1::2, 1] = np.where(unionMask, yVertex, nodes[:, 1::2, 1])

    # -------------------------------------------------------------------------
    # NOTE: I think this only applies to the final edge node, will it cause
    #       problems if it's applied to other edge nodes?
    # -------------------------------------------------------------------------
